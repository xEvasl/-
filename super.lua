-- Universal RotUp! Premium (single-source for server & client)
-- Paste THE SAME code:
--   • Script        → ServerScriptService   (server logic auto-runs)
--   • LocalScript   → StarterPlayerScripts  (client UI auto-runs)
-- No external modules. Server-side cloning for full parity with placement systems.

-- =========================
-- ======  SETTINGS  =======
-- =========================
local SETTINGS = {
    USE_SERVER_DUPLICATION = true,     -- server creates & equips clones (recommended)
    DELETE_ALL_CLONES       = false,    -- true = delete all clones; false = only latest
    TOAST_DURATION          = 2.0,      -- seconds
    TOGGLE_KEY              = Enum and Enum.KeyCode and Enum.KeyCode.G or nil, -- safe on server
    UI = {
        Title = "RotUp! Premium",
    }
}
...
if RunService:IsServer() then
    ...
    local function ensureHandle(tool)
        ...
        -- найдём первый BasePart и назначим его Handle
        local candidate
    ...
    -- SERVER DUPLICATE: creates perfect clone and equips it so platform sees it
    REQ_DUPLICATE.OnServerEvent:Connect(function(player, equippedRef)
        ...
        if backpack then
            ...
            if humanoid then
                humanoid:EquipTool(cloneRoot) -- важно: предмет сразу в руке (сервером)
            end
        else
            cloneRoot:Destroy()
        end
    ...
    -- SERVER DELETE: deletes latest or all clones marked IsClone=true
    ...
    return -- stop executing below on the server
end
...
if RunService:IsClient() then
    ...
    -- Remotes provided by server script
    ...
    titleLabel.Text = SETTINGS.UI.Title
    ...
    local deleteBtn = makeButton("Delete") -- ← имя кнопки удаления
    ...
    -- ==== Actions (server-only; no client fallback to avoid desync) ====
    local function duplicateAction()
        local equipped = getEquippedTool()
        if not equipped then showToast("Equip an item first."); return end
        if SETTINGS.USE_SERVER_DUPLICATION and REQ_DUPLICATE then
            REQ_DUPLICATE:FireServer(equipped)
            showToast("Requested server clone…")
        else
            showToast("Server remotes missing. Put this same file in ServerScriptService as Script.")
        end
    end

    local function deleteAction()
        if SETTINGS.USE_SERVER_DUPLICATION and REQ_DELETE then
            REQ_DELETE:FireServer(SETTINGS.DELETE_ALL_CLONES)
            showToast("Requested server delete…")
        else
            showToast("Server remotes missing.")
        end
    end
