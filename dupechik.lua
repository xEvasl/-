...
-- ===== DRAGGABLE UI ======
-- =========================
-- Drag by titleBar (mouse or touch). Keeps panel inside screen bounds.
...
local function clampToScreen(pos)
    -- Clamp panel top-left within viewport
    local vpSize = workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize or Vector2.new(1920,1080)
    local x = math.clamp(pos.X.Offset, -panel.AbsoluteSize.X + 20, vpSize.X - 20)
    local y = math.clamp(pos.Y.Offset, 20, vpSize.Y - 20)
...
local function updateDrag(input)
    ...
    panel.Position = clampToScreen(newPos)
end
...
-- Keep shadow aligned each frame
